...
export const InputField = ({
    ...
    const labelClasses =
        label &&
        (shouldFloat
            ? `absolute -top-2 left-3 bg-white px-1 text-[11px] font-medium z-10 transition-all duration-200 ${error ? "text-red-500" : isFocused ? "text-blue-500" : "text-gray-400"}`
        ...
        return (
            <div className="relative w-full">
                ...
                <div className="relative">
                    {isSelect ? (
                        ...
                        >
                            <option value="" disabled hidden>
                                {shouldFloat ? placeholder || "Chưa chọn" : ""}
                            </option>
                            {options.length > 0
                                ? options.map((opt) => (
                    ...
                    {type === "password" ? (
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none cursor-pointer z-10">
    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            ...
            <div className="relative">
                ...
                >
                    <span className={selectedOption ? "text-gray-700" : "text-gray-400"}>{selectedOption ? selectedOption.label : shouldFloat ? placeholder || "Chưa chọn" : ""}</span>
                ...
                {isOpen && (
                    <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-[100] overflow-hidden">
                        <div className="p-2 border-b border-gray-100 flex items-center gap-2 sticky top-0 bg-white">
                            ...
                            <input autoFocus type="text" placeholder="Tìm kiếm..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full text-sm outline-none py-1" onClick={(e) => e.stopPropagation()} />
                        </div>
                            ...
                            {filteredOptions.length > 0 ? (
                                filteredOptions.map((opt) => (
                                    <li
                                        ...
                                        className={`px-3 py-2 text-sm cursor-pointer hover:bg-blue-50 transition-colors ${opt.value === value ? "bg-blue-50 text-blue-600 font-medium" : "text-gray-700"}`}
                                    >
                                        {opt.label}
                                    </li>
                                ))

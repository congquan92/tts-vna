import type { Metadata } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";
import { AppRouterCacheProvider } from "@mui/material-nextjs/v16-appRouter";
import { AuthProvider } from "@/contexts/AuthContext";
import { Toaster } from "sonner";
// import { ThemeProvider } from "@mui/material/styles";
// import theme from "@/lib/theme"; // Nhớ check lại đường dẫn file theme

const geistSans = Geist({
    variable: "--font-geist-sans",
    subsets: ["latin"],
});

const geistMono = Geist_Mono({
    variable: "--font-geist-mono",
    subsets: ["latin"],
});

export const metadata: Metadata = {
    title: "Hệ thống quản lý Doanh nghiệp",
    description: "Ứng dụng quản lý doanh nghiệp với các tính năng quản lý nhân sự, tài chính, dự án và báo cáo. Tối ưu hóa quy trình làm việc và nâng cao hiệu quả kinh doanh.",
    icons: {
        icon: "/quochuy.ico",
    },
};

export default function RootLayout({
    children,
}: Readonly<{
    children: React.ReactNode;
}>) {
    return (
        <html lang="en" className={`${geistSans.variable} ${geistMono.variable} h-full antialiased`}>
            <body className="min-h-full flex flex-col">
                <AppRouterCacheProvider options={{ enableCssLayer: true }}>
                    <AuthProvider>
                        {/* <ThemeProvider theme={theme}>{children}</ThemeProvider> */}
                        {children}
                        <Toaster
                            position="top-right"
                            duration={5000}
                            closeButton={true}
                            richColors
                            toastOptions={{
                                classNames: {
                                    toast: "flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-sm",
                                    success: "bg-green-50 text-green-800 border border-green-200",
                                    error: "bg-red-50 text-red-800 border border-red-200",
                                    warning: "bg-yellow-50 text-yellow-800 border border-yellow-200",
                                    info: "bg-blue-50 text-blue-800 border border-blue-200",
                                    closeButton: "!absolute !left-auto !right-3 !top-5  !bg-transparent hover:!bg-black/10 !border-none !text-inherit cursor-pointer transition-colors !w-6 !h-6 flex items-center justify-center rounded-md",
                                },
                            }}
                        />
                    </AuthProvider>
                </AppRouterCacheProvider>
            </body>
        </html>
    );
}

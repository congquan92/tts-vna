...
export class UserService {
    ...
    async createUser(dto: CreateUserDto) {
        ...
        return await this.userRepository.createFullUser(
            userPayload,
            {
                username,
    ...
    async importFromExcel(file: Express.Multer.File) {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            try {
                ...
                const roleId = Number(row['Vai trò']);
                ...
                if (existedUsername) {
                    rowErrors.push('Tài khoản đã tồn tại');
                }
                ...
                if (!role) {
                    rowErrors.push('Vai trò không tồn tại');
                }
                ...
                errors.push({
                    row: index + 2,
                    data: row,
                    errors: ['Lỗi hệ thống khi import'],
                });
            }
        }
